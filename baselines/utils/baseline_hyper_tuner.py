...
# For imputation dataset, add pearson r2 score metric
def getPerSnpProbs(ProbWords, mask , win_size):
    """
    When predicting KMer words, retur the snp probabilities by marginalizing across all posssible
    ...
    """
    ProbWords_new = ProbWords[...,None]
    ProbWords_new=np.repeat(ProbWords_new,win_size,axis=3)
    mask = mask[None,:,:]
    ProbWords_new = ProbWords_new*mask
    prob = np.sum(ProbWords_new, axis=1)
    return prob.reshape(ProbWords.shape[0],-1)

def compute_r2_Kmer(predProbs, true_label, win_size, class_map, filepath):
    """
    PredProbs: list of (n_columns), each column is (n_samples,n_classes,)/ n_classes=6
    true_label:list of (n_columns), each column is (n_samples,n_classes,)/ n_classes=34
    class_map: list of (n_columns), each column np_array OHE 6x33
    """
    KmerMask = np.array(list(map(list, itertools.product([0, 1], repeat=win_size))))
    n_columns = len(predProbs)
    true_label_Cols = []            # list of (n_columns), each col n_samples,n_classes=33
    predProb_Cols = []              # list of (n_columns), each col n_samples,n_classes=33
    for i in range(n_columns):
        predProb_Col=predProbs[i]                   # n_samples,n_classes=6
        true_label_Col=true_label[i]                # n_samples,n_classes=34
        true_label_Col = true_label_Col[:,:-1]      # n_samples,n_classes=33
        n_classes = true_label_Col.shape[1]         # 33
        # RR:Test GB Kmer; comment 3 lines below
        # predProb_Col = predProb_Col[...,None]       # n_samples,n_classes=6,1
        # predProb_Col=np.repeat(predProb_Col,n_classes,axis=2)   # n_samples,n_PredClasses=6,n_classes=33
        # predProb_Col = np.sum(predProb_Col * class_map[i][None,...],axis=1) # n_samples,n_classes=33
        true_label_Cols.append(true_label_Col)
        predProb_Cols.append(predProb_Col)

    predProbs=np.stack(predProb_Cols).transpose(1,0,2)      #n_samples,n_columns,n_classes
    true_label=np.stack(true_label_Cols).transpose(1,0,2)    #n_samples,n_columns,n_classes
    probs=getPerSnpProbs(predProbs[...,1:], KmerMask, win_size)
    true_label_per_snp = getPerSnpProbs(true_label[...,1:], KmerMask, win_size)
    pearsonR2 = pearsonr(probs.flatten(), true_label_per_snp.flatten())[0]**2
    # save probs array
    # save true_labels_per_snp array
    np.save(''.join([filepath, '/probs']), probs)
    np.save(''.join([filepath, '/true_label_per_snp']), true_label_per_snp)
    return pearsonR2

def compute_r2(pred, true_label, filepath):
    probs = np.stack(pred).transpose(1,0,2)[:,:,1]
    true_label_per_snp = np.stack(true_label).transpose(1,0,2)[:,:,1]
    pearsonR2 = pearsonr(probs.flatten(), true_label_per_snp.flatten())[0]**2
    # save probs array
    # save true_labels_per_snp array
    np.save(''.join([filepath, '/probs']), probs)
...
class BaselineHyperTuner:
    def __init__(
        ...
        # for imputation
        if self.c.data_set=='imputation':
            self.y_pred_ls, self.y_test_ls, self.class_map=[],[],[]
    ...
    def run_hypertuning(self):
        ...
        for model_index, (model_name, model_classes) in enumerate(
            ...
            for cv_index in range(self.n_cv_splits):
                ...
                # calculate accumulated pearson r2 after every cv
                if self.c.data_set=='imputation':
                    self.compute_aggregate_pearson_r2()
    ...
    def run_cv_split_hypertuning(self):
        ...
        for i, col in enumerate(self.data_arrs):
            ...
            if i in self.non_target_cols:
                col = col[:, :-1]
                if self.c.data_set=='imputation' and self.c.data_Kmer:
                    col = np.argmax(col, axis=1).reshape(-1,1)
    ...
    def run_class_reg_hyper_tuning(self):
        ...
        if self.cat_target_cols:
            # # RR: Test GB Kmer
            # for class_col_index in self.cat_target_cols[0]:
            #     self.run_col_hyper_tuning(
            #         target_col=class_col_index, reg_class_mode='class')
            for class_col_index in self.cat_target_cols:
                self.run_col_hyper_tuning(
                    target_col=class_col_index, reg_class_mode='class')
            print(f'Total training time {sum(self.col_train_time)}')
            print(f'Len training time list: {len(self.col_train_time)}')
    ...
    def run_col_hyper_tuning(self, target_col, reg_class_mode,
        ...
        if y is None:
            ...
            if reg_class_mode == 'reg':
                ...
                if self.c.data_set=='imputation' : y_ohe = y.copy()
                y = np.argmax(y, axis=1)
                
        # for imputation dataset and XGBoost, all labels must be ints of form 0,1,2,...[num_class-1]
        # if self.c.data_set == 'imputation': y=y-1
        ...
        if self.c.data_set=='imputation' : 
            print(f"train classes, train_val_classes, test classes: {np.unique(self.y_train)}, {np.unique(self.y_train_val)},{np.unique(self.y_test)}")
            self.y_test_ohe = y_ohe[self.test_indices]
            if self.c.data_Kmer:
                num_classes_present=np.unique(self.y_train)
                class_map_arr=np.zeros((len(num_classes_present), y_ohe.shape[1]-1))#6x33
                # class_map_arr=np.zeros((33, y_ohe.shape[1]-1))#33x33 RR: for GB kMer
                for i,v in enumerate(num_classes_present):
                    class_map_arr[i,v]=1
                    # class_map_arr[v,v]=1    #RR: for GB kMer
                self.class_map.append(class_map_arr)

        compute_auroc = True
        #RR for comput_pearsonR2
        compute_pearsonrR2 = False
        if self.c.data_set=='imputation':
            compute_pearsonrR2 = True
        ...
        if self.reg_class_mode == 'class':
            ...
            if num_class_labels > 2 or (self.c.data_set=='imputation'):
                compute_auroc = False
                print('Disabling AUROC because multiclass.')
                if self.c.data_set=='imputation' and (self.c.sklearn_model == 'GradientBoosting'):
                    labels = self.labels = np.arange(33)
                else:
    ...
    def fit_eval_model(self, best_params, **kwargs):
        ...
        modelFit_start = time.time()
        model.fit(x, y, **kwargs)
        modelFit_end = time.time()
        print(f" model fit time per col for col {self.target_col} :{modelFit_end-modelFit_start} seconds")
        self.col_train_time.append(modelFit_end-modelFit_start)
        # save model
        ...
        if self.c.data_set=='imputation': 
            ...
            # if imputation, store y_pred and y_test for the end, so pearson r2 can be calculated at the end
            self.y_pred_ls.append(y_pred_probs)
            self.y_test_ls.append(self.y_test_ohe)
    ...
    def compute_aggregate_pearson_r2(self):
        filepath=osp.join(self.c.data_path, 'sklearn_imputation', f'model_class_{self.c.sklearn_model}' ,f'_num_win_{self.c.num_win}', f'_exp_n_runs_{self.c.exp_n_runs}')
        if not os.path.exists(filepath):
            os.makedirs(filepath)
        if (self.c.data_set=='imputation') and (self.c.data_Kmer):
            pearsonr2=compute_r2_Kmer(self.y_pred_ls, self.y_test_ls, self.c.Kmer_win_size, self.class_map, filepath)
        else:
            pearsonr2=compute_r2(self.y_pred_ls, self.y_test_ls, filepath)
        print(f"for cv_index: {self.cv_index}, pearson r2 for all columns is:{pearsonr2}")
        # empty the list to accumulate for next cv
        self.y_pred_ls, self.y_test_ls, self.class_map=[],[],[]
    
    def save_model(self, model):
        filepath=osp.join(self.c.data_path, 'sklearn_imputation', f'model_class_{self.c.sklearn_model}' ,f'_num_win_{self.c.num_win}', f'_exp_n_runs_{self.c.exp_n_runs}', '_model')
        if not os.path.exists(filepath):
            os.makedirs(filepath)
        with open(''.join([filepath, f'_col_no_{self.target_col}']), 'wb') as f:
            pickle.dump(model,f)