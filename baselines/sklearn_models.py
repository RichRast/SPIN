...
SKLEARN_CLASSREG_MODELS = {
    ...
    'GP': {
        'reg': partial(GaussianProcessRegressor, normalize_y=True),
    ...
    # 'SGP':{
    #     'reg': SparseGaussianProcessRegressor,
    # },
}
...
#GP Regression hypers
GPR_HYPERS={
    'random_state': [],  # Added from config
    "alpha": [0.1,0.5,1.0],
    "kernel":  [kernels.ConstantKernel(1.0, (1e-1, 1e3)) * kernels.RBF(1.0, (1e-3, 1e3)), kernels.ConstantKernel(1.0, (1e-1, 1e3)), kernels.RBF(1.0, (1e-3, 1e3))]
    
}

#GP Classification hypers
GPC_HYPERS={
    'random_state': [],  # Added from config
    "kernel": [kernels.ConstantKernel(1.0, (1e-1, 1e3)) * kernels.RBF(1.0, (1e-3, 1e3)), kernels.ConstantKernel(1.0, (1e-1, 1e3)), kernels.RBF(1.0, (1e-3, 1e3))]
...
#GP Classification hypers
SGP_HYPERS={
    "kernel":[kernels.DotProduct() + kernels.WhiteKernel(), kernels.ConstantKernel(1.0, (1e-1, 1e3)) * kernels.RBF(1.0, (1e-3, 1e3)), kernels.ConstantKernel(1.0, (1e-1, 1e3)), kernels.RBF(1.0, (1e-3, 1e3))]
}

KRR_HYPERS={
    "alpha": [0.1,0.5,1.0],
    # 'kernel_params':[{
    # "kernel__length_scale": [1e0, 1e-2, 1e2],
    # "kernel__periodicity":[1e0, 1e1]}],
    "kernel":["linear", kernels.ExpSineSquared(), kernels.ConstantKernel()]
}

BRR_HYPERS={
    'alpha_init':[1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1],
    'lambda_init': [1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1],
    'alpha_1':[1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1],
    'alpha_2':[1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1],
    'lambda_1':[1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1],
    'lambda_2':[1e-1, 1e-2, 1e-3, 1e-6, 1e-9, 1],
...
SMALL_DATASETS = [
        ...
        'energy-efficiency']
#RR: Add imputation in medium datasets
MEDIUM_DATASETS = [
    'poker-hand', 'protein', 'sarcos', 'income', 'kick', 'imputation']
LARGE_DATASETS = [
    'forest-cover', 'higgs']
...
def dataset_to_hypers(model_name, dataset_name):
    ...
    if model_name == 'MLP':
        ...
        elif dataset_name == 'imputation':
            print('Using Imputation MLP hypers.')
            return IMPUTATION_MLP_HYPERS
