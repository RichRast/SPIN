...
def encode_data(
    ...
    for col_index in range(D):
        ...
        if (col_index in cat_features) :
            ...
            elif c.data_Kmer or (c.data_Kmer and (c.model_class=='sklearn-baselines' and not c.sklearn_model=='XGBoost')):
                # RR: Added maximum categories for kMer and one-hot-encoding
                num_classes = int(math.pow(2, c.Kmer_win_size)) + 1
                encoded_col = np.zeros((non_missing_col.shape[0],num_classes))
                encoded_col[np.arange(non_missing_col.shape[0]),non_missing_col.flatten()] = 1
                encoded_col = encoded_col.astype(np.bool_)
            elif c.win_position:
                # RR: Added maximum categories for kMer and one-hot-encoding
                num_classes = 2
                encoded_col = np.zeros((non_missing_col.shape[0],num_classes))
                encoded_col[np.arange(non_missing_col.shape[0]),non_missing_col.astype(int).flatten()] = 1
                encoded_col = encoded_col.astype(np.bool_)
            else:
