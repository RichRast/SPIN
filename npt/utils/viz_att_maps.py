...
def viz_att_maps(c, dataset, wandb_run, cv_index, total_n_cv_splits):
    early_stop_counter = EarlyStopCounter(
        c=c, data_cache_prefix=dataset.model_cache_path,
        metadata=dataset.metadata, wandb_run=wandb_run, cv_index=cv_index, n_splits=total_n_cv_splits,
        device=c.exp_device)
    ...
    for name, param in model.named_parameters():
        if ('curr_att_maps' not in name) or (len(name.split('.'))<5):
            continue
    ...
    save_path = os.path.join(c.data_path, c.data_set, c.exp_descr, f'ssl__{c.model_is_semi_supervised}',
            f'np_seed={c.np_seed}__n_cv_splits={5}'
            f'__exp_num_runs={c.exp_n_runs}', c.model_checkpoint_key)
    ...
    for i in range(len(att_maps)):
        ...
        y_axis_factor=4
        x_axis_factor=0.1
        fig, axes = plt.subplots(ncols=n_heads, figsize=(15 * n_heads *x_axis_factor, 15*y_axis_factor))
        ...
        fig.savefig(os.path.join(save_path, f'layer_{i}'), dpi=fig.dpi)
        print(f"Attention maps saved at :{save_path}")
        plt.close(fig)
