...
INDUCTIVE_DISTILLED_DATASET = {
    # make the change such that test only sees test features Prod Class/Reg: makes sense to stratify train vs val vs test if poss #
    # # also means no stratified batching 
    (False, 'train'): ([0], False),
    (False, 'val'): ([1], False),
    (False, 'test'): ([2], False)
}

TRANSDUCTIVE_DISTILLED_DATASET = {
    # make the change such that test only sees test features Prod Class/Reg: makes sense to stratify train vs val vs test if poss #
    # # also means no stratified batching 
    (True, 'train'): ([0, 1, 2], True),
    (True, 'val'): ([0, 1, 2], True),
...
class NPTBatchDataset(torch.utils.data.IterableDataset):
    ...
    def get_batch_indices(self, dataset_mode):
        ...
        """
        if not (self.c.perceiver_D or self.c.perceiver_E or self.c.SPIN):
            batch_modes, mode_balance = BATCHING_SETTINGS_MAP[(
                self.c.model_is_semi_supervised,
                dataset_mode
            )]
        elif self.c.transductive_spin:
            batch_modes, mode_balance = TRANSDUCTIVE_DISTILLED_DATASET[(
                self.c.model_is_semi_supervised,
                dataset_mode
            )]
        else:
            batch_modes, mode_balance = INDUCTIVE_DISTILLED_DATASET[(
                self.c.model_is_semi_supervised,
                dataset_mode
            )]
    ...
    def batch_gen(self):
        ...
        if stratified_sampler and self.batch_size > 10:
            ...
            self.batch_sizes = None
        # print(f'row_index_order {self.dataset_mode}: {list(row_index_order)}')
