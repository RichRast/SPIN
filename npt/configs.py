...
def build_parser():
    ...
    parser.add_argument(
        '--project', type=str, default='DistilledNPT_v2',
        help='Wandb project name.')
    ###########################################################################
    # #### Data Config ########################################################
    ###########################################################################
    parser.add_argument(
        '--wandb_dir', type=str, default='/share/kuleshov/richras/npt/logs/',
        help='Directory to which wandb logs.')
    parser.add_argument(
        '--data_path', type=str, default='/share/kuleshov/richras/npt/data',
        help='Path of data')
    parser.add_argument(
        '--use_notebook', type='bool', default=False,
        help='set to true when using jupyter noteook fro argparse to dict')
    parser.add_argument(
        '--data_set', type=str, default='breast-cancer',
        help=f'Currently supported are breast-cancer, poker-hand, '
        f'higgs, epsilon, forest-cover, boston-housing, mnist, yacht, imputation '
    ...
    parser.add_argument(
        ...
        help='If True, delete logs from server and do not wandb.watch the '
             'model.')
    parser.add_argument(
        '--load_interim_data', default=False, type='bool',
        help='If True, save the data tabe npy files ')
    parser.add_argument(
        '--debug_variants', default=False, type='bool',
        help='If True, save the n closest known variants and the unknown snps ')
    parser.add_argument(
        '--debug_memory_profile', default=False, type='bool',
        help='If True, use pytorch memory profiler for cpu/gpu and print in log')
    parser.add_argument(
        '--showGPUUtil', default=False, type='bool',
        help='If True, print GPU utilization')
    parser.add_argument(
    ...
    #RR
    parser.add_argument(
        '--exp_descr', type=str, default='itx',
        help=f'description for cache path ')
    parser.add_argument(
        '--win_position', type='bool', default=False,
        help=f'add window position to data X ')
    parser.add_argument(
        '--subWin_size', type=int, default=1000,
        help=f'window size for unknown snps ')
    parser.add_argument(
        '--nClosest', type=int, default=150,
        help=f'nClosest snps')
    parser.add_argument(
        '--incr_nClosest', type=float, default=0.,
        help=f'increase sampling of known snps by this fraction')
    parser.add_argument(
        '--sample_nClosest', type=float, default=1.,
        help=f'use it with the above incr_nClosest. Fraction by which we'
        f'sample after increasing the sample by incr_nClosest amount')
    parser.add_argument(
        '--data_Kmer', type='bool', default=False,
        help=f'whether to chunk data in k-mers, where k = Kmer_win_size')
    parser.add_argument(
        '--Kmer_win_size', type=int, default=10,
        help=f'k of the k-mer')
    parser.add_argument(
        '--num_win', type=int, default=1,
        help=f'number of windows for imputation dataset')
    parser.add_argument(
        '--multiple_win', type='bool', default=False,
        help=f'set to True when running multiple win exp with imputation')
    parser.add_argument(
        '--old', type='bool', default=False,
        help=f'tmp flag to switch inference between old and new change')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default='NPT',  # in {NPT, sklearn-baselines, DKL,  MC_Dropout, ...}
        help='Specifies model(s) to train/evaluate.')
    parser.add_argument(
        '--set_transformer', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from set_transformer mab(X,H)')
    parser.add_argument(
        '--perceiver_A', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from perceiver_A mab(H,H)')
    parser.add_argument(
        '--perceiver_B', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from perceiver_B mab(H,H)')
    parser.add_argument(
        '--perceiver_C', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from perceiver_C mab(H,H)')
    parser.add_argument(
        '--perceiver_D', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from perceiver_D mab(H,H)')
    parser.add_argument(
        '--perceiver_E', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from perceiver_E mab(H,H)')
    parser.add_argument(
        '--SPIN', type='bool',
        default=False ,
        help='Specifies whether attention mechanism used is from SPIN mab(H,H)')
    parser.add_argument(
        '--onlyABD', type='bool',
        default=False ,
        help='all attention blocks are MAB(X,X) with attention between datapoints')
    parser.add_argument(
        '--onlyABA', type='bool',
        default=False ,
        help='all attention blocks are MAB(X,X) with attention between attributes')
    parser.add_argument(
        '--ABLA', type='bool',
        default=True ,
        help='uses self-attention on latent attribute H_A, set True by default')
    parser.add_argument(
        '--ABLA_HD', type='bool',
        default=False ,
        help='uses self-attention on latent attribute H_D, set False by default')
    parser.add_argument(
        '--enableEncoder', type='bool',
        default=False ,
        help='Enable encoder for perceiver implementation at inference mode')
    parser.add_argument(
        '--chunk_mask', type='bool',
        default=False ,
        help='chunk style masking by setting randome rows to mask instead of random mask tokens in label masking')
    parser.add_argument(
        '--dataSynthetic_query', type=int,
        default=3000 ,
        help='number of synthetic query points for train, val, test combined')
    parser.add_argument(
        '--dataSynthetic_n_rows', type=int,
        default=5000 ,
        help='number of synthetic rows for Aux')
    parser.add_argument(
        '--dataSynthetic_n_cols', type=int,
        default=50 ,
        help='number of synthetic cols')
    parser.add_argument(
        '--classesSynthetic', type=int,
        default=5 ,
        help='number of synthetic classes')
    parser.add_argument(
        '--bernoulliNoise', type=float,
        default=0.05 ,
        help='add bernoulli noise of this strength')
    parser.add_argument(
        '--decoderBoth', type='bool',
        default=False ,
        help='Use both XABA and ABAD for decoding')
    parser.add_argument(
        '--allow_even_only_stacking_depth', type='bool',
        default=True ,
        help='BY default allow for only even stacking depth')
    parser.add_argument(
        '--factor_embed_ABD', type=float,
        default=1.0 ,
        help='factor by which to reduce inner dim of embedding dim of XABD')

    parser.add_argument(
        '--grad_acc', type=int,
        default=1.0 ,
        help='if the value is >1, then we are doing gradient accumulation')
    parser.add_argument(
        '--train_proportion', type=float,
        default=1.0 ,
        help='proportion of training data to feed')
    ...
    parser.add_argument(
        ...
        help=f'If specified, run tuning for the particular '
             ...
             f'CatBoost, MLP, XGBoost, TabNet, LightGBM, KNN, GP'
    ...
    parser.add_argument(
        '--sparsemax',
        default=False,
        type='bool', help='use sparsemax instead of softmax')
        
    parser.add_argument(
        '--transductive_spin',
        default=True,
        type='bool', help='by default spin operates in transductive mode')

    parser.add_argument(
        '--fixed_test_set_index',
        default=104,
        type=int, help='specify last index for the test dataset ')
    parser.add_argument(
        '--no_test_data',
        default=False,
        type='bool', help='specify whether to use test data or not ')
    parser.add_argument(
        '--model_dtype',
        default='float32',
        type=str, help='Data type (supported for float32, float64) '
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help=f'number of inducing points that N is projected to I')
    parser.add_argument(
        '--num_inds_F',
        dest='num_inds_F',
        type=int,
        default=10,
        help=f'number of inducing points that features D is projected to F')
    parser.add_argument(
        '--startAttBlock',
        type=str,
        default='ABD',
        help=f'whether to start with ABD or ABA attention block')
    parser.add_argument(
        '--log_flops',
        type='bool',
        default=False,
