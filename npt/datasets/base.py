...
class BaseDataset(ABC):
    ...
    def get_data_dict(self, force_disable_auroc=None, dataset_iter=None, subWin_size=None):
        if not self.is_data_loaded:
            if dataset_iter is None:
                self.load() ##RR loads the data from the respective dataset class
            else:
                self.load(dataset_iter,subWin_size)
