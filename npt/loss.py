...
class Loss:
    ...
    * Loss.finalize_epoch_losses() to obtain the loss_dict for the current
    ...
    def __init__(
        ...
        self.use_pearsonR2=False
        if self.c.data_set=='imputation':
            self.use_pearsonR2=True
        self.setup_pearsonR2()
        self.reset()
    ...
    def setup_pearsonR2(self):
        if self.use_pearsonR2:
            print('Using pearsonR2 in loss module.')
            self.softmax = nn.Softmax(dim=1)
            self.reset_pearsonr2()
        else:
            print('Disabled pearsonR2 in loss module.')

    def reset_pearsonr2(self):
        """Reset helper variables for epoch-wide pearson r2 computation."""
        self._batch_r2_predictions = []
        self._batch_data_y = []
    ...
    def compute_loss(
        ...
        for col, (out, dat) in enumerate(zip(output, ground_truth_data)):
            #RR since the target is also cat, added or condition
            ...
            for mode, mode_loss_indices in loss_indices.items():
                ...
                loss, extra_out = self.compute_column_loss(
                    ...
                    num_preds=num_preds, sigma=sigma, mode = mode)  # RR Added mode as an argument for R2; will append output and pred for target cols
    ...
    def get_intermediate_epoch_losses(self, win_name=None):
        """
    ...
    def compute_column_loss(
        ...
        if is_cat:
            ...
            if self.use_pearsonR2 and mode == 'label':       # RR Added mode = lable to only get pred and output for target cols
                self._batch_r2_predictions.append(output[col_mask].detach())
                self._batch_data_y.append(data[col_mask].detach())
    ...
    def compute_r2(self):
        """Compute pearson r2 metric for predictions aggregated over batch."""
        if self.c.data_Kmer:
            
            preds = torch.cat(self._batch_r2_predictions, dim=0)
            preds=preds[:,1:] # ignore 0,1 as 0,1 is used for masking labels
            preds = self.softmax(preds)[:,:,None]
            assert torch.isclose(preds.sum(1), torch.ones_like(preds.sum(1))).sum(), "invalid Kmer softmax distribution"
            preds = preds.repeat(1,1,self.c.Kmer_win_size)
            KmerMask = torch.from_numpy(np.array(list(map(list, itertools.product([0, 1], repeat=self.c.Kmer_win_size)))))
            KmerMask = KmerMask[None,:,:].to(device=self.device)
            preds_masked = preds * KmerMask
            preds_masked = torch.sum(preds_masked, dim = 1)
            true_y = torch.cat(self._batch_data_y, dim=0)
            true_y = true_y[:,1:] # ignore 0,1 as 0,1 is used for masking labels
            true_y = true_y[:, :, None]
            true_y = true_y.repeat(1,1,self.c.Kmer_win_size)
            true_y_masked = true_y * KmerMask
            true_y_masked = torch.sum(true_y_masked, dim = 1)
            #assert that snp prob sum to 1
            assert torch.isclose(preds_masked + torch.sum(preds*(1-KmerMask), dim=1), torch.ones_like(preds_masked)).sum(), "snp_prob not sum to 1.0"
            r2 = pearsonr(preds_masked.cpu().detach().numpy().flatten(), true_y_masked.cpu().detach().numpy().flatten())[0]**2
            del preds, true_y, preds_masked, true_y_masked
            ...
            preds = self.softmax(preds)
            true_y = torch.cat(self._batch_data_y, dim=0)
            # get the true_y for 1 (as we compare r2 between p(snp_i=1) and if the actual value is 1)
            preds = preds[:,1]
            true_y = true_y[:,1]
            r2 = pearsonr(preds.cpu().detach().numpy().flatten(), true_y.cpu().detach().numpy().flatten())[0]**2
            del preds, true_y
