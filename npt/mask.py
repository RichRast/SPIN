...
def apply_mask(
    ...
    if (not c.model_is_semi_supervised) and (c.data_set not in ['synthetic', 'synthetic_att']):
...
def mask_data_for_dataset_mode(
    ...
    * Apply Bert-style augmentation masking on the features.
    ...
    if c.model_label_bert_mask_prob[dataset_mode] == 1:
        ...
        if c.data_set not in ['synthetic', 'synthetic_att']:
        ...
        if (label_prob == 0) and (dataset_mode == 'train'):
            ...
            if (c.data_set=="imputation") and (dataset_mode=="train") and (c.chunk_mask):
                masked_arrs, stochastic_mask_indices = apply_mask_imputation(
                data_arrs=input_arrs,
                mask_candidates=mask_candidates,
                cat_features=cat_features,
...
def apply_mask_imputation(
        data_arrs, mask_candidates, cat_features, mask_prob, c):
    """ Out of all the mask candidates, choose a certain percentage"
    "and mask them out by setting data_arr to 0 and mask token to 1 for train and val"
    "Returns:
    data_arrs: Now with masked entries.
    mask: Boolean torch.Tensor of shape (N, D). True for all masks that
        were set."""
    ...
    # We need to extract a list of all matrix entries with mask candidates.
    # mask_entries has shape (Nm, 2).
    mask_entries = torch.where(mask_candidates.sum(1)>0)[0]
    mask_cols = torch.where(mask_candidates.sum(0)>0)[0]
    Nm = len(mask_entries)  # number of mask candidates

    expected = mask_prob * Nm
    #choose indices to mask from 0 to 4291
    indices_to_choose = np.random.choice(
                np.arange(0, Nm),
                size=int(expected),
                replace=False)
    
    mask_row = mask_entries[indices_to_choose]
    # Iterate over all columns and set mask.
    for col, data_arr in enumerate(data_arrs):
        # Get boolean 'mask' selection mask for this column.
        # Zero out indices corresponding to
        ...
        if col in mask_cols:
            data_arr[mask_row, :]=0
            data_arr[mask_row,-1]=1
    # Reconstruct mask matrices from list of entries.
    mask = torch.zeros_like(mask_candidates).type(torch.bool)
    for c in mask_cols:
        mask[mask_row, c]=1
    return data_arrs, mask