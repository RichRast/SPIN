...
from npt.datasets.imputation import ImputationDataset
from npt.datasets.synthetic import SyntheticDataset
from npt.datasets.synthetic_att import SyntheticAttDataset
...
DATASET_NAME_TO_DATASET_MAP = {
    ...
    'kick': KickDataset,
    'imputation': ImputationDataset,
    'synthetic':SyntheticDataset,
    'synthetic_att':SyntheticAttDataset,
}
...
class ColumnEncodingDataset:
    ...
    """
    def __init__(self, c, device=None, **kwargs):
    ...
    def reset_cv_splits(self):
        self.dataset_gen = self.run_preprocessing_and_caching(self.dataset_iter)
    ...
    def init_cache_path_and_splits(self):
        ...
        cache_path = os.path.join(
            self.c.data_path, self.c.data_set, self.c.exp_descr, ssl_str,
    ...
    def get_data_dict(self, dataset_iter=None):
        # Get Data
        data_dict = self._dataset.get_data_dict( #RR based on the abstract class base.py, it fetches the dataset 
            force_disable_auroc=self.c.data_set in IMAGE_DATASETS, dataset_iter=self.dataset_iter, subWin_size=self.subWin_size)
    ...
    def run_preprocessing_and_caching(self,dataset_iter=None):
        """
        ...
        if not self.are_datasets_cached():
            ...
            data_dict = self.get_data_dict(dataset_iter)
    ...
    def generate_classification_regression_dataset(self, data_dict):
        ...
        for split_idx, (train_val_test_indices) in enumerate(
            ...
            if self.c.data_set in ['synthetic', 'synthetic_att']:
                
                for i, dataset_mode_rows in enumerate(new_train_val_test_indices):
                    if i==0:
                        train_mask_matrix=get_matrix_from_rows(
                        rows=dataset_mode_rows[-int(self.c.dataSynthetic_query/3):], # the last x rows are train query
                        cols=cat_target_cols,  
                        N=N, D=D)
                    elif i==1:
                        val_mask_matrix=get_matrix_from_rows(
                        rows=dataset_mode_rows,
                        cols=cat_target_cols,  
                        N=N, D=D)
                    else:
                        test_mask_matrix=get_matrix_from_rows(
                        rows=dataset_mode_rows,
                        cols=cat_target_cols,  
                        N=N, D=D)
            else:
