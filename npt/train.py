...
from npt.utils.dataUtil import getCudaFreeMem, getCudaPeakMem, Running_Average
...
class Trainer:
    def __init__(
        ...
        if c.data_set=='imputation':
            ...
            c=c, data_cache_prefix=dataset[0].model_cache_path,
            metadata=dataset[0].metadata, wandb_run=wandb_run, cv_index=cv_index,
            n_splits=min(dataset[0].n_cv_splits, c.exp_n_runs),
            device=self.gpu)
            # for random selection of a window during each epoch
            np.random.seed(c.np_seed)
            self.win = np.arange(len(self.dataset))
            np.random.shuffle(self.win)
            ...
            self.early_stop_counter = EarlyStopCounter(
                ...
                device=self.gpu)
        #RR
        self.early_stop_val_loss = 0.
        self.trainTimeRunningAvg=Running_Average()
        self.EvalTimeRunningAvg=Running_Average()
        # Initialize from checkpoint, if available
        num_steps = 0
        self.num_batches = self.get_num_steps_per_epoch()
    ...
    def per_epoch_train_eval(self, epoch):
        early_stop = False
        running_Avg_train_time_per_epoch=None
        running_Avg_train_time_steps=None
        if self.c.verbose:
        ...
        eval_model = end_experiment or self.eval_check(epoch) #RR: check for evaluation after every n epochs
        ...
        if self.c.debug_eval_row_interactions:
            ...
            train_loss, _ = self.run_epoch(dataset_mode='train', epoch=epoch,
                                        eval_model=False)
            train_end_time=time.time()
            self.trainTimeRunningAvg.update(train_end_time-train_start_time,1)

        if self.trainTimeRunningAvg.steps == 10:
            running_Avg_train_time_per_epoch=self.trainTimeRunningAvg()
            running_Avg_train_time_steps = self.trainTimeRunningAvg.steps
            self.trainTimeRunningAvg.reset()

        if eval_model:
            early_stop = self.eval_model(
                train_loss, epoch, end_experiment, log_train_time=running_Avg_train_time_per_epoch, log_train_time_steps=running_Avg_train_time_steps)
    ...
    def eval_model(self, train_loss, epoch, end_experiment, **kwargs):
        """Obtain val and test losses."""
        print(f"train_loss, epoch:{train_loss}, {epoch}")
        epoch_kwargs = dict(epoch=epoch, eval_model=True)
        running_Avg_eval_time_steps = None
        # Evaluate over val rows
        val_loss, win_name = self.run_epoch(dataset_mode='val', **epoch_kwargs)

        if not (self.c.debug_eval_row_interactions and epoch == 2):
            if self.c.data_set=='imputation':
                self.early_stop_val_loss = -val_loss[f'label_win_{win_name}']['r2']
            else:
                self.early_stop_val_loss = val_loss['label']['total_loss']
            # Early stopping check -- TODO: consider loss other than label?
            counter, best_model_and_opt = self.early_stop_counter.update(
                val_loss=self.early_stop_val_loss,
                model=self.model,
        ...
        if not self.c.debug_eval_row_interactions:
            if (counter == EarlyStopSignal.STOP) or end_experiment:
                ...
                # update val loss
                val_loss, _ = self.run_epoch(dataset_mode='val', **epoch_kwargs)

        if self.EvalTimeRunningAvg.steps == 10:
            running_Avg_eval_time_per_epoch=self.EvalTimeRunningAvg()
            running_Avg_eval_time_steps = self.EvalTimeRunningAvg.steps
            self.EvalTimeRunningAvg.reset()
        ...
        if train_loss is None and not self.c.debug_eval_row_interactions:
            ...
            train_loss, _ = self.run_epoch(dataset_mode='train', **epoch_kwargs)
            eval_end_time=time.time()
            self.EvalTimeRunningAvg.update(eval_end_time-eval_start_time,1)
        ...
        if ((counter == EarlyStopSignal.STOP)
            ...
            test_loss, _ = self.run_epoch(dataset_mode='test', **epoch_kwargs)
        else:
            test_loss = None
        ...
        #update loss_dict with peak memory usage
        if kwargs.get('log_train_time') is not None:
            loss_dict['Running_Avg_train_time_per_epoch']=kwargs.get('log_train_time')
            loss_dict['Running_Avg_train_time_steps']=kwargs.get('log_train_time_steps')
        if running_Avg_eval_time_steps is not None:
            loss_dict['Running_Avg_eval_time_per_epoch']=running_Avg_eval_time_per_epoch
            loss_dict['Running_Avg_eval_time_steps']=running_Avg_eval_time_steps
        loss_dict['peak_mem_Gb'] = getCudaPeakMem()
        if self.c.log_flops:
            loss_dict['thop_macs'] = self.macs
            loss_dict['thop_params'] = self.model_params
    ...
    def run_epoch(self, dataset_mode, epoch, eval_model=False):
        ...
        if self.c.data_set=='imputation':
            for i in range(len(self.dataset)):
                self.dataset[i].set_mode(mode=dataset_mode, epoch=epoch)
        else:
            self.dataset.set_mode(mode=dataset_mode, epoch=epoch)

        # Initialize data loaders (serial / distributed, pinned memory)
        dataset_idx = None # set this for genotype imputation dataset
        if self.is_distributed:
            ...
            if self.c.data_set=='imputation':
                remainder_idx = int(epoch%len(self.dataset))
                dataset_idx = self.win[remainder_idx]
                print(f" epoch {epoch} in {dataset_mode} mode and choosing window {dataset_idx}")
                batch_dataset = self.dataset[dataset_idx].cv_dataset 
            else:
            ...
            if (eval_model and self.c.debug_eval_row_interactions
                ...
                if self.c.debug_eval_row_interactions_timer is not None:
                    if batch_index % 50 == 0:
                        ...
                        loss_dict = self.loss.get_intermediate_epoch_losses(win_name=dataset_idx)
                        loss_dicts = {
                            'train_loss': {},
                            'val_loss': {},
                            'test_loss': {}}
                ...
                # Normal execution
                if self.c.showGPUUtil and epoch>0: print(f"GPU free, reserve, allocated before forward pass, epoch, dataset_mode:{epoch}, {dataset_mode}, {getCudaFreeMem()}")
                self.run_batch(
                    batch_dict_, dataset_mode, eval_model,
                    epoch, print_n, batch_index)
                if self.c.showGPUUtil and epoch>0: print(f"GPU free, reserve, allocated after forward pass, epoch:{epoch}, {dataset_mode}, {getCudaFreeMem()}")
        # Perform batch GD?
        ...
        if eval_model or batch_GD:
            ...
            loss_dict = self.loss.finalize_epoch_losses(eval_model, win_name=dataset_idx)

        # (See docstring) Either perform full-batch GD (as here)
        # or mini-batch SGD (in run_batch)
        NUM_ACCUMULATION_STEPS = int(self.c.grad_acc)
        if (not eval_model) and batch_GD:
            # Backpropagate on the epoch loss
            train_loss = loss_dict['total_loss']
            if NUM_ACCUMULATION_STEPS>1: train_loss = train_loss/NUM_ACCUMULATION_STEPS
            self.scaler.scale(train_loss).backward()
            if ((epoch + 1) % NUM_ACCUMULATION_STEPS == 0) or ((epoch+1)==self.max_epochs):
        ...
        if (not eval_model) and self.c.exp_minibatch_sgd:
            loss_dict = None
        del batch_dataset, batch_iter, batch_dict_
        torch.cuda.empty_cache()
        return loss_dict, dataset_idx
    ...
    def run_batch(self, batch_dict, dataset_mode, eval_model,
        ...
        if not self.c.data_set_on_cuda:
            ...
            # RR: tensors are placed on gpu here
        ...
        if (dataset_mode == 'train' and self.c.exp_minibatch_sgd
            ...
            if self.c.showGPUUtil and epoch>0: 
                print(f"GPU free, reserve, allocated before backward pass, epoch:{epoch}, {dataset_mode}, {getCudaFreeMem()}")
            # Scales loss.
            ...
            if ((batch_index + 1) % NUM_ACCUMULATION_STEPS == 0) or (batch_index + 1 == self.num_batches):
                self.scaler.step(self.optimizer)
        ...
        del ground_truth_tensors, masked_tensors, forward_kwargs
        del label_mask_matrix, augmentation_mask_matrix
        torch.cuda.empty_cache()
    ...
    def forward_and_loss(
        ...
        extra_args = {'dataset_mode':dataset_mode}
        ...
        if eval_model:
            ...
            output = self.model(masked_tensors, **extra_args)
            if (self.c.log_flops) and (self.flops_log_counter<10) and (dataset_mode=="train"): # Do this 10 times
                self.macs, self.model_params = profile(self.model, inputs=(masked_tensors, ))
                self.flops_log_counter+=1
